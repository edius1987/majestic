...
alias windowssync='rsync -aAXvz --exclude={"*.sys","\$Recycle.Bin/*","Exile*","Program Files \(x86\)/*","Program Files/*","Windows Defender/*","System Volume Information/*","AppData/Local/Temp/*"}'
...
#################
function wal-tile
	setenv wall eval (cat {$HOME}/.cache/wal/wal)
	wal -n -i "$argv"
	feh --bg-fill $wall
end
